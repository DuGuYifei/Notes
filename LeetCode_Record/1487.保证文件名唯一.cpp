...
{
public:
    string addNum(const string &name, int cnt)
    {
    ...
    {
        unordered_map<string, int> hash_names;
        int n = names.size();
        vector<string> ans(n);
        for (int i = 0; i < n; i++)
        {
            if (hash_names[names[i]])
            {
                string tmpName = addNum(names[i], hash_names[names[i]]++);
                while(hash_names[tmpName])
                    tmpName = addNum(names[i], hash_names[names[i]]++);
                ans[i] = tmpName;
            }
            else
                ans[i] = names[i];
