...
public:
    vector<string> alertNames(vector<string> &keyName, vector<string> &keyTime)
    {
        ...
        sort(id.begin(), id.end(), [n = keyName.data(), t = keyTime.data()](const int a, const int b)
             { return n[a] == n[b] ? t[a] < t[b] : n[a] < n[b]; });
        int prev = id[0];
        int prevOfPrev = -1;
        int cnt = 0;
        for (int i = 1; i < n; i++)
        {
            int cur = id[i];
            if (keyName[cur] == keyName[prev] && (ans.size() == 0 || keyName[cur] != ans.back()) && checkTime(keyTime[prev], keyTime[cur]))
            {
                cnt++;
                if (cnt == 2)
                {
                    if(checkTime(keyTime[prevOfPrev], keyTime[cur])){
                        cnt = 0;
                        ans.emplace_back(keyName[cur]);
                    }else
                        cnt--;
                }
            }
    ...
    bool checkTime(const string prev, const string cur)
    {
        int prevHour = stoi(prev.substr(0, 2));
        int curHour = stoi(cur.substr(0, 2));
        if (curHour - prevHour == 0)
            return true;
        if (curHour - prevHour > 1)
            return false;
        int prevMin = stoi(prev.substr(3, 2));
        int curMin = stoi(cur.substr(3, 2));
        if (curMin - prevMin <= 0)
            return true;
        return false;
    }
