...
class Solution {
    public int waysToMakeFair(int[] nums) {
        ...
        for(int i = 0; i < n; i++){
            if(i % 2 != 0){
                int even = suffix[0] - suffix[i + 1] + suffix[i + 2];
                int odd = suffix[1] - suffix[i] + suffix[i + 1];
                ans += odd != even? 0 : 1;
            }else{
                int even = suffix[0] - suffix[i] + suffix[i + 1];
                int odd = suffix[1] - suffix[i + 1] + suffix[i + 2];
                ans += odd != even? 0 : 1;
            }
        }
        return ans;
