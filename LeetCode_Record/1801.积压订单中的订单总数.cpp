 ...
 * [1801] 积压订单中的订单总数
 *
 * https://leetcode-cn.com/problems/number-of-orders-in-the-backlog/description/
 *
 * algorithms
 * Medium (39.21%)
 * Likes:    85
...
public:
    int getNumberOfBacklogOrders(vector<vector<int>>& orders) {
        ...
        for(auto& i : orders){
            if(i[2] == 0){
                while(!sell.empty()){
                    if(sell.top().first <= i[0]){
                        if(sell.top().second < i[1]){
                            i[1] -= sell.top().second;
                            sell.pop();
                            if(i[1] == 0){
                                break;
                ...
                while(!buy.empty()){
                    if(buy.top().first >= i[0]){
                        if(buy.top().second < i[1]){
                            i[1] -= buy.top().second;
                            buy.pop();
                            if(i[1] == 0){
                                break;
