...
public:
    ...
    inline string email(string& s){
        int idx = s.find_first_of('@');
        transform(s.begin(), s.end(), s.begin(), ::tolower);
        return s.substr(0, 1) + "*****" + s.substr(idx - 1, 1) + s.substr(idx);
    }
    
    inline string phone(const string& s){
        string t;
        for(auto c : s){
            if(isdigit(c))
                t.append(1, c);
        ...
        if(t.size() == 10)
            return "***-***-" + t.substr(6);
        return "+" + string(t.size()-10, '*') + "-***-***-" + t.substr(t.size()-4);
    }
};
// @lc code=end
