...
![](_attachments/old/2022-11-07-04-41-18.png) ![](_attachments/old/2022-11-07-04-41-30.png)

### Replication
如果您使用多台服务器，则可以选择复制。 在复制中，有一个主节点（服务器），与主节点存储相同数据的从节点（副本）连接到该主节点（服务器）。

![](_attachments/old/2022-11-07-04-42-41.png)
...
### 集群
![](_attachments/old/2022-11-07-04-44-39.png)

更高级的选项是 Redis 集群。 它不仅允许数据的复制（replication），还**允许数据在不同节点（多个主节点）之间的分布**。

1. 如果此配置中的一个主要节点丢失，这样一个节点的**副本会自动提升为主节点**。
![](_attachments/old/2022-11-07-04-45-49.png) ![](_attachments/old/2022-11-07-04-46-01.png)

2. 如果每个主节点的副本数相差一个以上，副本会自动重新平衡（迁移）。
![](_attachments/old/2022-11-07-04-46-53.png)
...
#### 基础
![](_attachments/old/2022-11-07-05-02-10.png) 
* DEL - 删除密钥 
* EXISTS - 检查密钥是否存在 
* EXPIRE - 设置密钥生命周期 
* KEYS - 查找匹配模式的密钥 
...
![](_attachments/old/2022-11-07-05-03-11.png)

#### SET
![](_attachments/old/2022-11-07-05-02-37.png)
...
#### List
![](_attachments/old/2022-11-07-05-04-48.png)
实现列表的可能方法是数组或链表。  Redis 使用链表。

![](_attachments/old/2022-11-07-05-05-03.png)
* LLEN - 列表的长度
* LPOP - 获取左侧的项目并将其删除
...
#### 哈希表 hash
![](_attachments/old/2022-11-07-05-06-10.png)
Hash或Hash maps非常适合存储对象（在对编程语言的理解中）。 对象属性的名称可以存储在键下，属性的值可以存储在值下。

![](_attachments/old/2022-11-07-05-06-38.png)
* HSET - 在给定的哈希图中设置多个字段
* HGET - 获取所选字段的值
* HGETALL - 获取所有字段值对
* HDEL - 删除指定的字段
...
#### 其他
![](_attachments/old/2022-11-07-05-08-16.png)
...
### Other features
![](_attachments/old/2022-11-07-05-09-15.png)
