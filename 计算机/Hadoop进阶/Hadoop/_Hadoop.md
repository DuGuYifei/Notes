# hadoop

## 基础
[ANDB](../../数据库/NoSQL/ANDB/_AdvancedNonrelationalDatabase.md)

## 进阶
1. [MapReduce流程](MapReduce流程.md)
2. [Hadoop VSCode Environment](Hadoop_VSCodeEnvvironment.md)
3. [Mapper和Reducer的泛型](Mapper和Reducer的泛型.md)
4. [WritableComparable接口](WritableComparable接口.md)
5. [setup_clenup_run_context](setup_clenup_run_context.md)
6. [map和reduce执行次数](map和reduce执行次数.md)
7. [map按行读取](map按行读取.md)
8. [map和reduce里context.write有null值](map和reduce里context.write有null值.md)
9. [读取文hdfs里的文件](读取文hdfs里的文件.md)
10. [通过args参数和confiuration传递参数](通过args参数和confiuration传递参数.md)
11. [Hadoop命令运行jar](Hadoop命令运行jar.md)
12. [不想每次手动删除输出文件夹](不想每次手动删除输出文件夹.md)
13. [Configured和Tool](Configured和Tool.md)
14. [设置百分比进度](设置百分比进度.md)