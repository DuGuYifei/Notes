...
- [1. 导言：模型、工具和智能体](#1-导言模型工具和智能体)
- [2. Tools and tool calling](#2-tools-and-tool-calling)
  - [2.1. 我们所说的“工具”是什么意思？](#21-我们所说的工具是什么意思)
  - [2.2. 工具的类型](#22-工具的类型)
    - [2.2.1. 函数工具（Function Tools）](#221-函数工具function-tools)
    - [2.2.2. 内置工具 (Built-in tools)](#222-内置工具-built-in-tools)
    - [2.2.3. Agent 工具 (Agent Tools)](#223-agent-工具-agent-tools)
  - [2.3. 代理工具分类学 (Taxonomy of Agent Tools)](#23-代理工具分类学-taxonomy-of-agent-tools)
  - [2.4. 最佳实践](#24-最佳实践)
    - [2.4.1. 文档说明至关重要](#241-文档说明至关重要)
    - [2.4.2. 描述动作，而非实现 (Describe actions, not implementations)](#242-描述动作而非实现-describe-actions-not-implementations)
    - [2.4.3. 发布任务，而非 API 调用 (Publish tasks, not API calls)](#243-发布任务而非-api-调用-publish-tasks-not-api-calls)
  - [2.5. 使工具尽可能细粒度 (Make tools as granular as possible)\*](#25-使工具尽可能细粒度-make-tools-as-granular-as-possible)
    - [2.5.1. 设计简洁的输出 (Design for concise output)](#251-设计简洁的输出-design-for-concise-output)
    - [2.5.2. 有效地使用验证 (Use validation effectively)](#252-有效地使用验证-use-validation-effectively)
    - [2.5.3. 提供描述性的错误消息 (Provide descriptive error messages)](#253-提供描述性的错误消息-provide-descriptive-error-messages)
- [3. 理解模型上下文协议（Understanding the Model Context Protocol）](#3-理解模型上下文协议understanding-the-model-context-protocol)
  - [3.1. “N x M”集成问题与标准化的必要性](#31-n-x-m集成问题与标准化的必要性)
  - [3.2. 核心架构组件：宿主、客户端和服务器 (Core Architectural Components: Hosts, Clients, and Servers)](#32-核心架构组件宿主客户端和服务器-core-architectural-components-hosts-clients-and-servers)
  - [3.3. 通信层：JSON-RPC、传输协议和消息类型 (The Communication Layer: JSON-RPC, Transports, and Message Types)](#33-通信层json-rpc传输协议和消息类型-the-communication-layer-json-rpc-transports-and-message-types)
  - [3.4. 核心基元：工具及其他 (Key Primitives: Tools and others)](#34-核心基元工具及其他-key-primitives-tools-and-others)
    - [3.4.1. 工具 (Tools)](#341-工具-tools)
      - [3.4.1.1. 工具定义 (Tool Definition)](#3411-工具定义-tool-definition)
      - [3.4.1.2. 工具结果 (Tool Results)](#3412-工具结果-tool-results)
        - [3.4.1.2.1. 非结构化内容 (Unstructured Content)](#34121-非结构化内容-unstructured-content)
        - [3.4.1.2.2. 结构化内容 (Structured Content)](#34122-结构化内容-structured-content)
        - [3.4.1.2.3. 错误处理 (Error Handling)](#34123-错误处理-error-handling)
    - [3.4.2. 其他能力 (Other Capabilities)](#342-其他能力-other-capabilities)
      - [3.4.2.1. 资源 (Resources)](#3421-资源-resources)
      - [3.4.2.2. 提示词 (Prompts)](#3422-提示词-prompts)
      - [3.4.2.3. 采样 (Sampling)](#3423-采样-sampling)
      - [3.4.2.4. 启发 (Elicitation)](#3424-启发-elicitation)
      - [3.4.2.5. 根目录 (Roots)](#3425-根目录-roots)
- [4. 模型上下文协议：支持与反对 (Model Context Protocol: For and Against)](#4-模型上下文协议支持与反对-model-context-protocol-for-and-against)
  - [4.1. 能力与战略优势 (Capabilities and Strategic Advantages)](#41-能力与战略优势-capabilities-and-strategic-advantages)
    - [4.1.1. 加速开发并培育可复用的生态系统 (Accelerating Development and Fostering a Reusable Ecosystem)\*\*](#411-加速开发并培育可复用的生态系统-accelerating-development-and-fostering-a-reusable-ecosystem)
    - [4.1.2. 动态增强智能体能力与自主性 (Dynamically Enhancing Agent Capabilities and Autonomy)](#412-动态增强智能体能力与自主性-dynamically-enhancing-agent-capabilities-and-autonomy)
    - [4.1.3. 架构灵活性与面向未来 (Architectural Flexibility and Future-Proofing)](#413-架构灵活性与面向未来-architectural-flexibility-and-future-proofing)
    - [4.1.4. 治理与控制的基础 (Foundations for Governance and Control)](#414-治理与控制的基础-foundations-for-governance-and-control)
    - [4.1.5. 关键风险与挑战 (Critical Risks and Challenges)](#415-关键风险与挑战-critical-risks-and-challenges)
    - [4.1.6. 性能与可扩展性瓶颈 (Performance and Scalability Bottlenecks)](#416-性能与可扩展性瓶颈-performance-and-scalability-bottlenecks)
    - [4.1.7. 企业就绪差距（Enterprise Readiness Gaps）](#417-企业就绪差距enterprise-readiness-gaps)
- [5. MCP 中的安全性 (Security in MCP)](#5-mcp-中的安全性-security-in-mcp)
  - [5.1. 新的威胁格局 (New threat landscape)](#51-新的威胁格局-new-threat-landscape)
  - [5.2. 风险与缓解措施 (Risks and Mitigations)](#52-风险与缓解措施-risks-and-mitigations)
  - [5.3. 主要风险与缓解措施 (Top Risks \& Mitigations)](#53-主要风险与缓解措施-top-risks--mitigations)
    - [5.3.1. 动态能力注入 (Dynamic Capability Injection)](#531-动态能力注入-dynamic-capability-injection)
      - [5.3.1.1. 风险 (Risk)](#5311-风险-risk)
      - [5.3.1.2. 缓解措施 (Mitigations)](#5312-缓解措施-mitigations)
    - [5.3.2. 工具阴影 (Tool Shadowing)](#532-工具阴影-tool-shadowing)
      - [5.3.2.1. 风险 (Risk)](#5321-风险-risk)
      - [5.3.2.2. 示例场景 (Example scenario)：](#5322-示例场景-example-scenario)
      - [5.3.2.3. 缓解措施 (Mitigations)](#5323-缓解措施-mitigations)
    - [5.3.3. 恶意工具定义与消耗内容 (Malicious Tool Definitions and Consumed Contents)](#533-恶意工具定义与消耗内容-malicious-tool-definitions-and-consumed-contents)
      - [5.3.3.1. 风险 (Risk)](#5331-风险-risk)
      - [5.3.3.2. 缓解措施 (Mitigations)](#5332-缓解措施-mitigations)
    - [5.3.4. 敏感信息泄露 (Sensitive information Leaks)](#534-敏感信息泄露-sensitive-information-leaks)
      - [5.3.4.1. 风险 (Risk)](#5341-风险-risk)
      - [5.3.4.2. 缓解措施 (Mitigations)](#5342-缓解措施-mitigations)
    - [5.3.5. 不支持限制访问范围 (No support for limiting the scope of access)](#535-不支持限制访问范围-no-support-for-limiting-the-scope-of-access)
      - [5.3.5.1. 风险 (Risk)](#5351-风险-risk)
      - [5.3.5.2. 缓解措施 (Mitigations)](#5352-缓解措施-mitigations)
- [6. 结论 (Conclusion)](#6-结论-conclusion)
- [7. 附录 (Appendix)](#7-附录-appendix)
  - [7.1. 混淆代理问题 (Confused Deputy problem)](#71-混淆代理问题-confused-deputy-problem)


## 1. 导言：模型、工具和智能体

如果没有外部功能的访问权限，即使是最先进的基础模型也只是一个模式预测引擎。先进模型可以出色地完成许多任务——通过法律考试、编写代码或诗歌、创建图像和视频、解决数学问题，但其自身只能根据先前训练的数据生成内容。除了请求上下文中输入的信息外，它无法访问有关世界的任何新数据；它无法与外部系统交互；也无法采取任何行动来影响其环境。

为了解决这一局限性，大多数现代基础模型现在都具备了调用外部函数或“工具”的能力。就像智能手机上的应用程序一样，工具使 AI 系统不仅仅能生成模式。这些工具充当了 Agent 的“眼睛”和“手”，使其能够感知世界并采取行动。

随着代理式 AI 的出现，工具对 AI 系统变得更加重要。AI Agent 利用基础模型的推理能力与用户互动并为其实现特定目标，而外部工具则赋予了 Agent 这种能力。具备采取外部行动的能力后，Agent 可以对企业级应用产生巨大影响。

然而，将外部工具连接到基础模型面临着重大挑战，既包括基础技术问题，也涉及重要的安全风险。模型上下文协议（Model Context Protocol, MCP）于2024年推出，作为简化工具与模型集成过程并解决其中一些技术和安全挑战的方式。

在本白皮书中，我们首先探讨基础模型所使用的工具属性：它们是什么以及如何使用。我们提供了设计高效工具和有效使用工具的最佳实践与指南。随后，我们审视模型上下文协议，讨论其基本组件以及相关的挑战和风险。最后，我们深入分析在企业环境中引入 MCP 并将其连接到高价值外部系统时所带来的安全挑战。
...
### 2.3. 代理工具分类学 (Taxonomy of Agent Tools)
...
#### 2.4.2. 描述动作，而非实现 (Describe actions, not implementations)
...
#### 2.4.3. 发布任务，而非 API 调用 (Publish tasks, not API calls)
...
### 2.5. 使工具尽可能细粒度 (Make tools as granular as possible)*
...
#### 2.5.1. 设计简洁的输出 (Design for concise output)
...
#### 2.5.2. 有效地使用验证 (Use validation effectively)
...
#### 2.5.3. 提供描述性的错误消息 (Provide descriptive error messages)
...
## 3. 理解模型上下文协议（Understanding the Model Context Protocol）

### 3.1. “N x M”集成问题与标准化的必要性
...
### 3.2. 核心架构组件：宿主、客户端和服务器 (Core Architectural Components: Hosts, Clients, and Servers)
...
### 3.3. 通信层：JSON-RPC、传输协议和消息类型 (The Communication Layer: JSON-RPC, Transports, and Message Types)
...
### 3.4. 核心基元：工具及其他 (Key Primitives: Tools and others)
...
###### 3.4.1.2.2. 结构化内容 (Structured Content)
...
## 4. 模型上下文协议：支持与反对 (Model Context Protocol: For and Against)
...
### 4.1. 能力与战略优势 (Capabilities and Strategic Advantages)

#### 4.1.1. 加速开发并培育可复用的生态系统 (Accelerating Development and Fostering a Reusable Ecosystem)**
...
#### 4.1.2. 动态增强智能体能力与自主性 (Dynamically Enhancing Agent Capabilities and Autonomy)
...
#### 4.1.3. 架构灵活性与面向未来 (Architectural Flexibility and Future-Proofing)
...
#### 4.1.4. 治理与控制的基础 (Foundations for Governance and Control)
...
#### 4.1.5. 关键风险与挑战 (Critical Risks and Challenges)
...
#### 4.1.6. 性能与可扩展性瓶颈 (Performance and Scalability Bottlenecks)
...
#### 4.1.7. 企业就绪差距（Enterprise Readiness Gaps）
...
## 5. MCP 中的安全性 (Security in MCP)

### 5.1. 新的威胁格局 (New threat landscape)
...
### 5.2. 风险与缓解措施 (Risks and Mitigations)

在广泛的 MCP 安全威胁格局中，有几项关键风险因其特别突出而值得识别。

### 5.3. 主要风险与缓解措施 (Top Risks & Mitigations)

#### 5.3.1. 动态能力注入 (Dynamic Capability Injection)
...
#### 5.3.3. 恶意工具定义与消耗内容 (Malicious Tool Definitions and Consumed Contents)
...
#### 5.3.4. 敏感信息泄露 (Sensitive information Leaks)
...
#### 5.3.5. 不支持限制访问范围 (No support for limiting the scope of access)
...
### 7.1. 混淆代理问题 (Confused Deputy problem)
