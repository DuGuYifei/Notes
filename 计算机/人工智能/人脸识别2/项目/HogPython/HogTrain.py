...
for i, line in enumerate(f):
    if i == 1:
        ...
        person = [lines[-8], lines[-7], lines[-6], lines[-5], lines[-4], lines[-3], lines[-2], lines[-1]]
        ppl.append(person)
        lines = lines[0:num_I]
...
def split_cells(mag, dir):
    ...
    for row in cell_begin:
        for col in cell_begin:
            hist = np.zeros(NUM_BINS)
            for i in range(row, row + CELL_SIZE_X):
                for j in range(col, col + CELL_SIZE_X):
    ...
    print(histograms)
    return np.array(histograms).reshape(1, NUM_BINS * int(img_size / CELL_SIZE_X) * int(img_size / CELL_SIZE_X))
