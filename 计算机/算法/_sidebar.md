- [返回上级目录](../_sidebar.md)
- [位运算](位运算.md)
- 其他算法
    - [Dijkstra](其他算法/Dijkstra.md)
    - [Dijkstra_Bellman-Ford](其他算法/Dijkstra_Bellman-Ford.md)
    - [_其他算法](其他算法/_其他算法.md)
    - [凸包算法_Jarvis算法_Graham算法_Andrew算法](其他算法/凸包算法_Jarvis算法_Graham算法_Andrew算法.md)
    - [最小费用最大流](其他算法/最小费用最大流.md)
- 基本算法
    - [0到2中的另外两个](基本算法/0到2中的另外两个.md)
    - [2的n次幂](基本算法/2的n次幂.md)
    - [AC自动机](基本算法/AC自动机.md)
    - [BFS](基本算法/BFS.md)
    - [BFS和Dijkstra](基本算法/BFS和Dijkstra.md)
    - [BinaryLifting](基本算法/BinaryLifting.md)
    - [KMP](基本算法/KMP.md)
    - [NP问题](基本算法/NP问题.md)
    - [Tire_前缀树_字典树](基本算法/Tire_前缀树_字典树.md)
    - [_基本算法](基本算法/_基本算法.md)
    - [sliding-window滑动窗口算法](基本算法/sliding-window滑动窗口算法.md)
    - [二分法](基本算法/二分法.md)
    - [二进制中1的个数](基本算法/二进制中1的个数.md)
    - [判断数字异号](基本算法/判断数字异号.md)
    - [判断数字某一位或最后一位](基本算法/判断数字某一位或最后一位.md)
    - [判断超出整数界限](基本算法/判断超出整数界限.md)
    - [判断链表的环问题](基本算法/判断链表的环问题.md)
    - [前缀和和差分](基本算法/前缀和和差分.md)
    - [动态规划&记忆搜索&回溯&dfs](基本算法/动态规划&记忆搜索&回溯&dfs.md)
    - [取余](基本算法/取余.md)
    - [取反运算](基本算法/取反运算.md)
    - [四叉树](基本算法/四叉树.md)
    - [回溯算法](基本算法/回溯算法.md)
    - [大小写转换_位运算](基本算法/大小写转换_位运算.md)
    - [字符串匹配](基本算法/字符串匹配.md)
    - [字符串相加搜索](基本算法/字符串相加搜索.md)
    - [完全二叉树和二进制的关系](基本算法/完全二叉树和二进制的关系.md)
    - [将最后一个1变为0](基本算法/将最后一个1变为0.md)
    - [小于和不等于的速度](基本算法/小于和不等于的速度.md)
    - [并查集](基本算法/并查集.md)
    - [排序算法](基本算法/排序算法.md)
    - [摩尔投票](基本算法/摩尔投票.md)
    - [数值的一半_右移1位](基本算法/数值的一半_右移1位.md)
    - [最大公约数GCD](基本算法/最大公约数GCD.md)
    - [最小公倍数LCM](基本算法/最小公倍数LCM.md)
    - [树状数组](基本算法/树状数组.md)
    - [水塘抽样](基本算法/水塘抽样.md)
    - [求最后一个1后面的后缀](基本算法/求最后一个1后面的后缀.md)
    - [状态压缩](基本算法/状态压缩.md)
    - [确定一个数每比特位都是1](基本算法/确定一个数每比特位都是1.md)
    - [红黑树](基本算法/红黑树.md)
    - [约瑟夫环问题](基本算法/约瑟夫环问题.md)
    - [线段树](基本算法/线段树.md)
    - [记忆搜索](基本算法/记忆搜索.md)
    - [质因数](基本算法/质因数.md)
    - [质数](基本算法/质数.md)
    - [跳表](基本算法/跳表.md)
    - [遍历的不同方式](基本算法/遍历的不同方式.md)
    - [随机众数](基本算法/随机众数.md)
- [算法](算法.md)
- 算法错题集
    - [_算法错题集](算法错题集/_算法错题集.md)
    - [k分为n互斥组_2022分为10个数](算法错题集/k分为n互斥组_2022分为10个数.md)